...
export const clinicMedicineService = {
  getAll: async (params?: { searchTerm?: string; clinicId?: number; isActive?: boolean }): Promise<ClinicMedicine[]> => {
    // Build params object, only including defined values
    // When isActive is undefined, don't include it so backend receives null (shows all)
    const queryParams: Record<string, string | number | boolean> = {}
    if (params?.searchTerm) queryParams.searchTerm = params.searchTerm
    if (params?.clinicId !== undefined) queryParams.clinicId = params.clinicId
