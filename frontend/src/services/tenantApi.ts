  ...
  return {
    ...
    auth: {
      login: (credentials: { email: string; password: string }) => 
        tenantAxios.post<ApiResponse<{ token: string; user: TenantUser }>>('/auth/tenant/login', credentials),
      logout: () => {
        localStorage.removeItem('tenant_token');
        localStorage.removeItem('tenant_user');
      },
      getProfile: () => 
        tenantAxios.get<ApiResponse<TenantUser>>('/auth/tenant/profile'),
      updateProfile: (data: Partial<TenantUser>) => 
        tenantAxios.put<ApiResponse<TenantUser>>('/auth/tenant/profile', data)
    }
  };
};
