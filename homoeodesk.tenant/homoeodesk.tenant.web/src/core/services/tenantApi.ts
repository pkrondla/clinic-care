...
import type { Clinic, ClinicFilters } from '../types/branch';
  ...
  return {
    ...
    Branches: {
      getAll: (filters?: ClinicFilters) => 
        tenantAxios.get<PaginatedResponse<Clinic>>('/branches', { params: filters }),
      getById: (id: number) => 
        tenantAxios.get<ApiResponse<Clinic>>(`/Branches/${id}`),
      create: (data: Partial<Clinic>) => 
        tenantAxios.post<ApiResponse<Clinic>>('/branches', data),
      update: (id: number, data: Partial<Clinic>) => 
        tenantAxios.put<ApiResponse<Clinic>>(`/Branches/${id}`, data),
      delete: (id: number) => 
        tenantAxios.delete<ApiResponse<void>>(`/Branches/${id}`)
    },
