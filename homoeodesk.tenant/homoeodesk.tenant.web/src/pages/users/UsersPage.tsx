...
export const UsersPage = () => {
  ...
  const BranchesList: Clinic[] = Array.isArray(Branches) ? Branches : []
  ...
  const handleEdit = (user: UserType) => {
    ...
    form.setFieldsValue({
      ...
      BranchIds: user.clinicAccess?.map(ca => ca.BranchId) || [],
          ...
          {record.clinicAccess && record.clinicAccess.length > 0 ? (
            <Space wrap size={[4, 4]}>
              {record.clinicAccess.slice(0, 2).map(ca => (
                <Tag key={ca.BranchId}>{ca.clinicName}</Tag>
              ))}
