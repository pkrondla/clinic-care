...
import type { Clinic, OperatingHoursType } from '@core/services/branchService';
...
export const DoctorSchedulePage = () => {
  ...
  const { data: doctors = [] } = useDoctors({ BranchId: selectedClinic?.id });
  ...
  const BaseBranch = selectedDoctor?.BaseBranchId 
    ? Branches.find(c => c.id === selectedDoctor.BaseBranchId) 
    : null;

  const handleAdd = () => {
    setEditingRecord(null);
    setSelectedAvailabilityType(AvailabilityType.DifferentBranch);
    form.resetFields();
    form.setFieldsValue({
      availabilityType: AvailabilityType.DifferentBranch,
    });
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (values.availabilityType === AvailabilityType.Leave) {
        ...
        request.BranchId = values.BranchId || selectedClinic?.id;
      } else {
        request.BranchId = values.BranchId;
  ...
  const getAvailabilityTypeConfig = (type: AvailabilityType) => {
    const configs = {
      ...
      [AvailabilityType.DifferentBranch]: { color: 'blue', text: 'Different Clinic', icon: '📍' },
  ...
  const columns = [
    ...
    },
    {
      ...
      filters: [
        ...
        { text: 'Different Clinic', value: AvailabilityType.DifferentBranch },
  ...
  return (
    <div>
      <Card>
        ...
        {selectedDoctor && BaseBranch && (
          <Alert
            message="Default Schedule"
            description={
              <div>
                <Text strong>{selectedDoctor.doctorName}</Text> is normally available at <Text strong>{BaseBranch.name}</Text> during clinic operating hours.
                <br />
                Add exceptions below for different Branches, leaves, or modified hours.
              </div>
            }
            type="info"
        ...
        >
              ...
              {doctors.map((doctor) => (
                <Select.Option key={doctor.id} value={doctor.id}>
                  <Space>
                    ...
                    {doctor.BaseBranchName && (
                      <Text type="secondary" style={{ fontSize: '12px' }}>
                        (Base: {doctor.BaseBranchName})
                      </Text>
                    )}
                  </Space>
                </Select.Option>
          ...
          >
            ...
            >
              <Space direction="vertical">
                <Radio value={AvailabilityType.DifferentBranch}>
                  <Space direction="vertical" size={0}>
          ...
          {selectedAvailabilityType === AvailabilityType.Leave ? (
            ...
            <>
              ...
              <Form.Item
                label={
                  selectedAvailabilityType === AvailabilityType.DifferentBranch 
                    ? 'Visiting Clinic' 
                    : 'Clinic'
                }
                name="BranchId"
