...
export const LoginPage = () => {
  ...
  const onFinish = async (values: LoginFormValues) => {
    ...
    try {
      ...
      const availableBranches = loginData.availableBranches || loginData.AvailableBranches || []
      ...
      if (accessToken && userData) {
        ...
        const user: User = {
          ...
          organizationName: userData.organizationName || userData.OrganizationName || 'HomoeoDesk',
          selectedBranchId: userData.selectedBranchId || userData.SelectedBranchId,
          selectedBranchName: userData.selectedBranchName || userData.SelectedBranchName,
          availableBranches: availableBranches.map((c: { id?: number; Id?: number; name?: string; Name?: string; code?: string; Code?: string }) => ({
            id: c.id || c.Id!,
            name: c.name || c.Name || '',
            code: c.code || c.Code || ''
          }))
        ...
        login(user, accessToken, user.availableBranches)
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <Title level={2} style={{ marginBottom: 8 }}>
            {isTenantDomain ? `${subdomain}` : 'HomoeoDesk'}
          </Title>
          <Text type="secondary">
