    ...
    public async Task<Result<InvoiceDto>> Handle(CreateInvoiceCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var clinic = await _context.Clinics
                .FirstOrDefaultAsync(c => c.Id == request.ClinicId && c.OrganizationId == organizationId.Value && c.IsActive, cancellationToken);
            if (clinic == null)
            {
            ...
            foreach (var item in request.Items)
            {
                if (item.ItemType.Equals("Medicine", StringComparison.OrdinalIgnoreCase) && item.MedicineId.HasValue)
                {
                    ...
                    {
                        _logger.LogWarning(
                            "Inventory not found for medicine {MedicineId} in clinic {ClinicId}",
                            item.MedicineId.Value, request.ClinicId);
                    }
                }
            }
