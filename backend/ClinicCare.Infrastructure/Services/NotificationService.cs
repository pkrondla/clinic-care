...
public class NotificationService : INotificationService
{
    ...
    public async Task SendAppointmentReminderAsync(int appointmentId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var clinicAddress = appointment.Clinic?.Address ?? "";
    ...
    public async Task SendPrescriptionReadyNotificationAsync(int prescriptionId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (prescriptionEntity == null || prescriptionEntity.Patient?.User == null)
            {
                return;
            }

            var organizationId = prescriptionEntity.OrganizationId;
            var patientId = prescriptionEntity.PatientId;
            var patientName = prescriptionEntity.Patient.User.FullName;
            var doctorName = prescriptionEntity.Consultation?.Doctor?.User?.FullName ?? "Doctor";
            var clinicName = prescriptionEntity.Consultation?.Clinic?.Name ?? "Clinic";
            var clinicHours = GetClinicHours(prescriptionEntity.Consultation?.Clinic);
    ...
    public async Task SendInvoiceNotificationAsync(int invoiceId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var clinicName = invoiceEntity.Clinic?.Name ?? "Clinic";
    ...
    /// </summary>
    private string GetClinicHours(Domain.Entities.Clinic? clinic)
    {
        if (clinic == null)
    ...
    {
        var clinicName = variables.TryGetValue("ClinicName", out var name) ? name : "Clinic";
        
        return notificationType switch
        {
            NotificationType.AppointmentCreated => $"Appointment Confirmed - {clinicName}",
            NotificationType.AppointmentReminder => $"Appointment Reminder - {clinicName}",
            NotificationType.AppointmentCancelled => $"Appointment Cancelled - {clinicName}",
            NotificationType.PrescriptionCreated => $"Prescription Generated - {clinicName}",
            NotificationType.PrescriptionReadyForCollection => $"Prescription Ready - {clinicName}",
            NotificationType.InvoiceCreated => $"Invoice Generated - {clinicName}",
            NotificationType.PaymentReceived => $"Payment Received - {clinicName}",
            NotificationType.CourierDispatched => $"Medicines Dispatched - {clinicName}",
            NotificationType.CourierDelivered => $"Medicines Delivered - {clinicName}",
            _ => $"Notification from {clinicName}"
        };
    }
    ...
    public async Task SendConsultationCompletedNotificationAsync(int consultationId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var clinicName = consultation.Clinic?.Name ?? "Clinic";
    ...
    public async Task SendFollowUpReminderNotificationAsync(int patientId, int doctorId, DateTime followUpDate, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Get clinic name from the most recent consultation or use default
            ...
            var clinicName = recentConsultation?.Clinic?.Name ?? "Clinic";
